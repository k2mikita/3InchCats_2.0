...
public class BasicBehaviour : MonoBehaviour
{
	...
	void Update()
	{
		...
		h = m * Input.GetAxis("Horizontal");
		v = m * Input.GetAxis("Vertical");
		...
		if (!attacking && !bashing && Input.GetMouseButtonDown(0))
        {
			attacking = true;
		...
		if (!bashing && !attacking && Input.GetMouseButtonDown(1))
		{
			bashing = true;
			anim.SetBool("Bashing", true);
		}
